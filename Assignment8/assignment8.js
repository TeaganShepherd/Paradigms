...
function stringFromArrays(arr1, arr2) {
	...
	let temp = "" + arr1[0] + arr2[0];
	arr1.shift();
	arr2.shift();
	return (temp + stringFromArrays(arr1,arr2));
}
...
function computeMaxArr(f1, f2) {
	...
	return function recurse(arr){
		if (arr.length <= 1){
			return Math.max(f1(arr[0]), f2(arr[0]));
		}

		let temp = arr.shift();
		console.log(temp);
		return  "" + Math.max(f1(temp), f2(temp)) +","+ recurse(arr);
	}
